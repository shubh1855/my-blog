...
The service runs a stripped 64-bit ELF binary that validates user input through a custom stack-based
virtual machine.
...
•  A 25×26 maze stored in a flat memory region starting at address 0x1000
...
The bytecode initializes wall cells using repeated PUSH_IMM32 → STORE_MAZE instructions.
 By mapping flat memory indices with a stride of 26, the maze grid is reconstructed.
...
R12 → D2 → L2 → D2 → L2 → U2 → L4 → D2 → R2 → D2 →
L6 → D2 → R6 → D6 → R2 → D4 → R4 → D2 → L6 → U2 →
L4 → U2 → L2 → D6 → R2 → U2 → R2 → D2 → R10 → U2 →
R2 → D2 → R4 → U2 → R2 → D2

Expanded as a single contiguous input string sent to the service:

RRRRRRRRRRRRDDLLDDLLUULLLLDDRRDDLLLLLL
DDRRRRRRDDDDDDRRDDDDRRRRDDLLLLLLUULLLL
UULLDDDDDDRRUURRDDRRRRRRRRRRUURRDDRRRR
UURRDD
...
Upon reaching the goal cell, the VM executes its flag-read routine and prints the contents of
flag.txt:
